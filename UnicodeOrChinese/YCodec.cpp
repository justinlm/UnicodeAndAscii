...
// hex to char
// error: return -1 
char YCodec::Dec2Char(unsigned int n)
{
	char ch;
    if(/*n>=0 &&*/ n<=9)	ch = 48 + n;
...
// string to hex code
// error: return -1 
long YCodec::String2Dec(char* string, int strlen)
{
	long hex=-1;
	int i=0, n=0;
	char *p = string;
...
// error: return -1
int YCodec::Dec2String(long hex, char* string, int* slen)
{
	char tmp[11] = {0};
	if(hex < 0)	return -1;
	if(string == NULL){// count the length it will be used
        sprintf(tmp, "%x",  (unsigned int)hex);
		*slen = strlen(tmp);
		return 1;
	}
	memset(string, 0, *slen);
    sprintf(string, "%x", (unsigned int)hex);
...
int YCodec::HexStr2WChars(char *hexstr, int hexstrlen, int eachchar, wchar_t *wchs, int *wchsLen)
{
	...
	for(int i=0; i<hexstrlen; i+=eachchar){
		...
		// char* to hex
        long hex = String2Dec(tmp, strlen(tmp));
		if(hex == -1)	continue;
		// hex to wchar_t
		*pwch++ = Hex2Wchart(hex);
	}
...
// wchar_t to hex code
long YCodec::Wchart2Dec(wchar_t wch)
{
	return (long)wch;
}
...
int YCodec::Wchart2HexStr(wchar_t *wchs, int wchslen, int eachchar, char* hexstr, int* hexstrlen)
{
	...
	for(int i=0; i<wchslen; i++){
		// wchar_t to hex
        long hex = Wchart2Dec(*pwch++);
		// hex to char*
		int charlen = 0;
        if(Dec2String(hex, NULL, &charlen) != -1){
			char* str = new char[charlen+1];
			memset(str, 0, charlen+1);
            int n = Dec2String(hex, str, &charlen);
			if(n != -1){
				int k=0;
				memset(tmp, 0, eachchar+1);
